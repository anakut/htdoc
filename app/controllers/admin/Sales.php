...
// ==========================================end================================
    ...
    public function p_view($id = NULL, $view = TRUE, $save_bufffer = NULL)
    {
        ...
        if ($view) {
            ...
            //     $this->load->view($this->theme . 'sales/invoice_up', $this->data);
            // }else{
            //     $this->load->view($this->theme . 'sales/invoice_upSecond', $this->data);
            // }
...
// --------------------------------------end priner--------------------------
    ...
    public function add($quote_id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            // $t_dis = $this->input->post('order_discount');
        ...
        if ($rows) {
            ...
            foreach ($rows as $row) {
                ...
                $pr[] = array('id' => ($c + $r), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'category' => $row->category_id, 
                    'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'units' => $units,'sale_unit'=>$saleUnit, 'options' => $options,'location'=>$row->location);
                $r++;
            }
    ...
    public function get_sale_payment(){
        ...
        foreach ($cus_payment as $pay) {
            ...
            $tdata.="<tr>";
                ...
                $tdata.="<td>".number_format($pay->grand_total-$pay->paid,2)."($)</td>";
            $tdata.="</tr>";
