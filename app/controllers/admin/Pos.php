...
class Pos extends MY_Controller
{
    ...
    public function getProductDataByCode($code = NULL, $warehouse_id = NULL, $customer_id=NULL)
       {
        ...
        if ($row) {
            ...
            $row->comment = '';
            $row->sale_unitprice = $row->sale_unitprice;
            $row->puid = $row->puid;
            $row->spid = $row->spid;
            $combo_items = false;
            if ($row->type == 'combo') {
            ...
            $units = $this->site->getUnitsByBUID($row->base_unit);
            $tax_rate = $this->site->getTaxRateByID($row->tax_rate);
            $saleUnit = $this->site->getSaleUnitBySUID($row->puid);
            $pr = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'category' => $row->category_id, 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'units' => $units,'sale_unit' =>$saleUnit , 'options' => $options,'location' =>$row->location);
    ...
    public function ajaxproducts($category_id = NULL, $brand_id = NULL)
    {
        ...
        if (!empty($products)) {
            foreach ($products as $product) {
                ...
                $prods .= "<button id=\"product-" . $category_id . $count . "\" type=\"button\" value='" . $product->code . "' title=\"" . $product->name . "\" class=\"btn-prni btn-" . $this->pos_settings->product_button_color . " product pos-tip\" data-container=\"body\"><img src=\"" . base_url() . "assets/uploads/thumbs/" . $product->image . "\" alt=\"" . $product->name . "\" class='img-rounded' /><span style='font-family: Khmer OS ;'>" . character_limiter($product->name, 40) . "</span></button>";
    ...
    public function ajaxcategorydata($category_id = NULL)
    {
        ...
        if ($subcategories) {
            foreach ($subcategories as $category) {
                $scats .= "<button id=\"subcategory-" . $category->id . "\" type=\"button\" value='" . $category->id . "' class=\"btn-prni subcategory\" ><img src=\"" . base_url() ."assets/uploads/thumbs/" . ($category->image ? $category->image : 'no_image.png') . "\" class='img-rounded img-thumbnail'  /><span style='font-family: Khmer OS ;'>" . $category->name . "</span></button>";
            }
        }
    ...
    public function view($sale_id = NULL, $modal = NULL)
    {
        ...
        // print_r($this->data['customer']);die();
        ...
        if (!empty($bills)) {
            ...
            foreach ($bills as $bill) {
                ...
                $html .= '<p class="hide">' . $bill->suspend_note . '</p>
                          ...
                          <p class="">Total: $' . $this->sma->formatMoney($total->subtotal) . '</p>';
