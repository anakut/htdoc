...
class Site extends CI_Model
{
    ...
    public function getAllTables(){
        ...
        // $this->db->group_by('group_name','table');
        $re = $this->db->get();
        if ($re->num_rows() > 0) {
            return $re->result();
        }
    ...
    function getnewavgcost($id,$amount,$quantity,$warehouse_id){
        
        $qty_old=$this->getproductquantity($id,date('Y-m-d H:i:s'),$warehouse_id);
        $old_cost=$this->getcost($id,date('Y-m-d H:i:s'),$warehouse_id);
        $new_quantity=0;//$row->qty;
        $new_amount=0;//$row->amount;
        $cost=(($qty_old*$old_cost)+$amount)/($qty_old+$quantity);
        // echo $cost; die();
        return $cost;
    }
    function getproductquantity($id,$date='2017-01-31',$warehouse_id=''){
        // if(strtotime($date)>strtotime('2017-03-31')){
        
        $whs='';
        $whp='';
        if($warehouse_id!=''){
            $whs=" AND si.warehouse_id='$warehouse_id'";
            $whp=" AND warehouse_id='$warehouse_id'";
        }
        $tb2="(SELECT IFNULL(SUM(si.quantity), 0) as s_total FROM sma_sale_items si INNER JOIN sma_sales s ON (si.sale_id=s.id) WHERE si.product_id='$id' AND sale_status='completed' {$whs} AND date(s.`date`)<='$date') as S";
        return $this->db->query("SELECT P.p_total - S.s_total as quantity FROM 
        {$tb2},
        (SELECT IFNULL(SUM(quantity), 0) as p_total FROM sma_purchase_receive_item WHERE item_id='$id' {$whp} AND date(`date`)<='$date') as P")->row()->quantity;
      
    }
    function getcost($id,$date='2017-01-31',$warehouse_id){
       $row=$this->db->query("SELECT item_id,`date`,trans_date,cost as my_cost,'0' as new_adjust_cost,'0' as adjust_cost,purchase_itemid FROM sma_purchase_receive_item WHERE  item_id='$id' and warehouse_id='$warehouse_id' and date(`date`)<='$date' AND not_include=0  order by receive_item_id desc limit 1")->row();
            
            if(isset($row->my_cost)){
                if($row->adjust_cost==1){
                    return $row->new_adjust_cost;
                }
                return $row->my_cost;
            }else{
                $p_row=$this->db->where('id',$id)->get('products')->row();
                return $p_row->cost;
            }
        // }
    }
    function getwarehouse_stock($product_id,$warehouse_id){
        $row = $this->db->where('product_id',$product_id)->where('warehouse_id',$warehouse_id)->get('warehouses_products')->row();
    ...
    public function get_customer_payment_alert(){
        $sql = $this->db->query("SELECT scp.name,scp.company,scp.address,scp.phone,ssp.customer_id, ssp.date,scp.cf1,sum(ssp.grand_total) as gtotal,sum(ssp.paid) as paid_total
                FROM sma_sales ssp 
                LEFT JOIN sma_companies scp
                ON scp.id = ssp.customer_id
                WHERE ssp.payment_status <> 'paid' AND scp.group_name ='customer' and scp.cf1 <> 0 and date_format(DATE_ADD(ssp.date, INTERVAL `scp`.`cf1` DAY),'%y-%m-%d')<=date_format(CURDATE(),'%y-%m-%d')
                GROUP BY ssp.customer_id
                ");
        if ($sql->num_rows() > 0) {
    ...
    public function get_count_customer_payment_alert(){
        $coun = $this->db->query("SELECT COUNT(DISTINCT ssp.customer) as cus_qty
                FROM sma_sales ssp 
                LEFT JOIN sma_companies scp
                ON scp.id = ssp.customer_id
                WHERE ssp.payment_status <> 'paid' AND scp.group_name ='customer' and scp.cf1 <> 0 and date_format(DATE_ADD(ssp.date, INTERVAL `scp`.`cf1` DAY),'%y-%m-%d')<=date_format(CURDATE(),'%y-%m-%d')
     ...
     public function getSaleUnitBySUID($sale_unit){
        $this->db->where('product_id', $sale_unit)
        ->group_by('id')->order_by('id asc');
        $q = $this->db->get("saleunitprice");
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
     ...
     public function getEditValueByID($id)
    {
        $this->db->select('*');
        $this->db->from('saleunitprice');
